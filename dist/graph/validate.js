...
export function validatePatterns(nodes) {
    const issues = [];
    for (const n of nodes) {
        const ev = n.fm.evidence;
        if (!ev)
            continue;
        for (const [kind, patterns] of [["grep", ev.grep], ["git", ev.git]]) {
            for (const p of patterns ?? []) {
                try {
                    new RegExp(p);
                }
                catch (e) {
                    issues.push({ level: "error", file: n.file, message: `evidence.${kind} 正则无效: ${p}（${e.message}）` });
                }
            }
        }
    }
