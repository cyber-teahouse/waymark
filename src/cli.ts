...
export function createProgram(): Command {
  ...
  const rootOf = (cmd: Command): string =>
    cmd.opts<{ root?: string }>().root ?? program.opts<{ root: string }>().root;
  ...
  withRoot(program.command("sync"))
    ...
    .action(async (_opts: unknown, cmd: Command) => {
      ...
      console.log(
        `节点 ${s.total} | 完成 ${s.done} | 进行中 ${s.inProgress} | 未开始 ${s.planned} | 警示 ${s.warnings}`,
      );
      for (const i of issues) {
        console.log(`${i.level === "error" ? "✖" : "⚠"} [${i.level}] ${i.file}: ${i.message}`);
      }
  ...
  withRoot(program.command("done"))
    ...
    .action((id: string, opts: { note?: string; acc?: boolean }, cmd: Command) => {
      try {
        const root = rootOf(cmd);
  ...
  withRoot(program.command("block"))
    ...
    .action((id: string, opts: { note?: string }, cmd: Command) => {
      try {
        const root = rootOf(cmd);
  ...
  withRoot(program.command("reopen"))
    ...
    .action((id: string, opts: { planned?: boolean; note?: string }, cmd: Command) => {
      try {
        const root = rootOf(cmd);
    ...
    .action(async (opts: { fresh?: boolean }, cmd: Command) => {
      ...
      if (missing || stale) {
        if (opts.fresh) {
          ...
          console.warn(
            "⚠ plan/ 或证据目录在 sync 之后有改动，工作流数据可能过期——建议重新 waymark sync（或使用 --fresh）",
          );
        }
      }
      try {
